...
public class RepositoryTests
{
    ...
    public async Task AddAsync_Saves_And_Returns_Entity()
    {
        ...
        (await db.Documents.FirstAsync()).Id.Should().Be(entity.Id);
    }
    ...
    [Fact]
    public async Task GetAllAsync_Returns_In_UploadedAt_Order()
    {
        using var db = NewDb(Guid.NewGuid().ToString());
        var repo = new DocumentRepository(db);

        db.Documents.AddRange(
            new DocumentEntity { Id = Guid.NewGuid(), FileName = "b", ContentType = "x", SizeBytes = 1, UploadedAt = new DateTime(2023, 1, 1) },
            new DocumentEntity { Id = Guid.NewGuid(), FileName = "a", ContentType = "x", SizeBytes = 1, UploadedAt = new DateTime(2022, 1, 1) }
        );
    ...
    public async Task DeleteAsync_Removes_And_Returns_True()
    {
        ...
        var repo = new DocumentRepository(db);
        var id = Guid.NewGuid();
        db.Documents.Add(new DocumentEntity { Id = id, FileName = "x", ContentType = "y", SizeBytes = 1, UploadedAt = DateTime.UtcNow });
        await db.SaveChangesAsync();

        var removed = await repo.DeleteAsync(id);

        removed.Should().BeTrue();
        (await db.Documents.FindAsync(id)).Should().BeNull();
    }
